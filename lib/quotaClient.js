...
module.exports = { fetchQuota, fetchWebStats, jwtExp, jwtType, ensureAccessToken, refreshAccessToken };
...
function jwtType(token) {
  const j = jwtPayload(token);
  return j && typeof j.typ === 'string' ? j.typ : null;
}
...
// [20260824 兼容 Kimi 会员接口字段] 网页端近期将部分 Connect RPC 的 JSON 字段
// 改为 snake_case；不同灰度版本仍可能返回旧 camelCase，故两种响应都接受。
function webField(record, camelName, snakeName) {
...
async function fetchWebStats(token, { timeoutMs = 8000, expectedUserId = null } = {}) {
  ...
  try {
    ...
    const subscriptionBalance = webField(j, 'subscriptionBalance', 'subscription_balance');
    const monthly = isRec(subscriptionBalance)
      ? {
          usedPercent: ratioToPct(webField(subscriptionBalance, 'amountUsedRatio', 'amount_used_ratio')),
          // 新接口额外返回 Kimi Code 在会员月度额度中的占比，保留给面板展示。
          codeUsedPercent: ratioToPct(
            webField(subscriptionBalance, 'kimiCodeUsedRatio', 'kimi_code_used_ratio')
          ),
          expireTime: webField(subscriptionBalance, 'expireTime', 'expire_time') || null,
        }
      : null;
    const giftBalances = webField(j, 'giftBalances', 'gift_balances');
    const gifts = Array.isArray(giftBalances)
      ? giftBalances.filter(isRec).map((g) => ({
          usedPercent: ratioToPct(webField(g, 'amountUsedRatio', 'amount_used_ratio')),
          expireTime: webField(g, 'expireTime', 'expire_time') || null,
        }))
      : [];
