...
//     （v2 起时间为 epoch 毫秒数字、workDir 改名 cwd，读取时已归一化）
...
const CACHE_VERSION = 4; // v4: 增加 byDayModel 交叉聚合（范围筛选）+ 刷新模型名映射
...
/**
 * 把时间字段归一化为 ISO 字符串。
 * state.json v1 用 ISO 字符串，v2（2026-08 起）改为 epoch 毫秒数字；
 * 下游排序、Date.parse 范围筛选、fmtDate 均按 ISO 字符串处理，故在此统一。
 * @param {unknown} v state.json 中的 createdAt / updatedAt 原始值
 * @returns {string|null} ISO 字符串，无法识别时为 null
 */
...
function readStateJson(sessionDir) {
  try {
    ...
    return {
      ...
      updatedAt: toIsoTime(s.updatedAt),
      // v2 起工作目录字段从 workDir 改名为 cwd
      workDir: s.workDir || s.cwd || '',
    };
  } catch {
...
function summarizeFile(filePath, stat, fallbackMs, displayOf) {
  const byDay = {};
  const byModel = {};
  const byDayModel = {}; // { day: { model: usage } }，范围筛选用
  ...
  for (const line of text.split('\n')) {
    ...
    byDayModel[dk] = byDayModel[dk] || {};
    byDayModel[dk][mk] = byDayModel[dk][mk] || EMPTY_USAGE();
    addUsage(byDayModel[dk][mk], obj.usage);
  }
  return {
    size: stat.size,
...
function scanSessions(homeDir, { recentLimit = 200, cache, modelDisplay } = {}) {
  ...
  for (const session of sessions) {
    ...
    for (const agent of fs.readdirSync(agentsDir)) {
      ...
      for (const [dk, models] of Object.entries(summary.byDayModel || {})) {
        result.byDayModel[dk] = result.byDayModel[dk] || {};
        for (const [mk, u] of Object.entries(models)) {
          result.byDayModel[dk][mk] = result.byDayModel[dk][mk] || EMPTY_USAGE();
          addUsage(result.byDayModel[dk][mk], u);
        }
      }
    }
